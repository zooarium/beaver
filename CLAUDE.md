...
- **Releases**: every service ships `CHANGELOG.md` (Keep a Changelog; `templates/service/CHANGELOG.md.tmpl`) and a `make release VERSION=x.y.z` target (rotates `[Unreleased]` into dated section, commits, tags `vX.Y.Z`).
